...
kj::Promise<void> WebSessionBridge::handleResponse(
  ...
  return promise.then([this,KJ_MVCAP(contextInitInfo),&out](
    ...
    if (options.isApi) {
      ...
      KJ_IF_MAYBE(cspMgr, cspManager) {
        if(!(*cspMgr)->isLegacyRelaxed()) {
          ...
          //   in the apps.
          // - There is some hackery around images and media; these have some
          //   legitimate use cases (e.g. embedding images in feeds in ttrss) and
          //   we want to provide a way for a user to allow these via the UI.
          //   We do this by setting a reporting uri and triggering a UI popup
          //   when we receive a csp violation report for img-src or media-src.
          bool allowMedia = (*cspMgr)->currentPolicy().getAllowMedia();
          kj::String wsHost;
          ...
          KJ_IF_MAYBE(hostStr, baseHost) {
            ...
            baseHttpHost = kj::str("http", secure, "://", *hostStr);
            wsHost = kj::str("ws", secure, "://", *hostStr);
            ...
            cspReport = kj::str(
                ...
                "report-to {\"group\":\"app-csp\",\"endpoints\": [{\"url\":\"",
                cspReportUri, "\"}],\"max_age\":10886400}"
            );
          }
#define UNSAFE "'unsafe-inline' 'unsafe-eval' data: blob:; "
          kj::StringPtr mediaPolicy;
          if(allowMedia) {
              mediaPolicy =
                "img-src * " UNSAFE
                "media-src * " UNSAFE;
